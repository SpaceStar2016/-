...
@interface CatalogVC ()<UITableViewDataSource,UITableViewDelegate>
@property (nonatomic, retain) NSMutableArray *totalArray;
@property (nonatomic, retain) UITableView *tableView;

@property (nonatomic, retain) NSArray<CatalogModel *> *dataArray;
...
- (void)viewDidLoad {
    ...
    self.dataArray = [CatalogModel mj_objectArrayWithKeyValuesArray:array];
...
- (void)didClickRightButton{
    ...
    for (NSInteger i = 0; i< self.dataArray.count; i++) {
        [self.dataArray[i].content enumerateObjectsUsingBlock:^(SubCatalogModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (void)didClickedSection:(UIControl *)view{
    ...
    /** 如果需要收起上一个分区 就用下面的代码 */
    //    [self.dataArray enumerateObjectsUsingBlock:^(CatalogModel * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    //    if (indexPath.row == 0) {
    CatalogCell *cell = [tableView dequeueReusableCellWithIdentifier:@"cell"];
...
- (UITableView *)tableView{
    if (!_tableView) {
        _tableView = [[UITableView alloc] initWithFrame:CGRectMake(0,kNavTopHeight, self.view.frame.size.width, self.view.frame.size.height - kNavTopHeight) style:(UITableViewStyleGrouped)];
        ...
        [_tableView registerNib:[UINib nibWithNibName:NSStringFromClass([CatalogCell class]) bundle:nil] forCellReuseIdentifier:@"cell"];
